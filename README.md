...
``` shell
python -m torch.distributed.launch --nproc_per_node 4 --master_port 9527 train.py --workers 8 --device 0,1,2,3 --sync-bn --batch-size 36 --data <data path (*.yaml)> --img 1280 1280 --cfg cfg/training/PRB_Series/PRB-FPN6-3PY.yaml --weights '' --name PRB-FPN6-3PY --hyp data/hyp.scratch.p5.yaml
```
...
```shell
python test.py --data <data path (*.yaml)> --img 1280 --batch 32 --conf 0.5 --iou 0.65 --save-txt --device 0 --weights prb-fpn.pt --name PRB-FPN6-3PY_Pseudo_label
```
